...
class BaseSynthesizer:
    ...
    def __getstate__(self):
        ...
        load the model even when used from an external tool such as ``SDV``.
        Also, if ``random_states`` are set, store their states as dictionaries rather than
        generators.
    ...
    def __setstate__(self, state):
        ...
        Restore the ``random_states`` from the state dict if those are present and
        then set the device according to the current hardware.
        """
